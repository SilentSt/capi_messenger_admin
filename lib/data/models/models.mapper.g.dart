...
class SignUpDtoMapper extends BaseMapper<SignUpDto> {
  ...
  SignUpDto fromMap(Map<String, dynamic> map) => SignUpDto(token: Mapper.i.$get(map, 'token'));

  @override Function get encoder => (SignUpDto v) => encode(v);
  dynamic encode(SignUpDto v) => toMap(v);
  Map<String, dynamic> toMap(SignUpDto s) => {'token': Mapper.i.$enc(s.token, 'token')};

  @override String stringify(SignUpDto self) => 'SignUpDto(token: ${Mapper.asString(self.token)})';
  @override int hash(SignUpDto self) => Mapper.hash(self.token);
  @override bool equals(SignUpDto self, SignUpDto other) => Mapper.isEqual(self.token, other.token);
...
class _SignUpDtoCopyWithImpl<$R> extends BaseCopyWith<SignUpDto, $R> implements SignUpDtoCopyWith<$R> {
  ...
  @override $R call({String? token}) => $then(SignUpDto(token: token ?? $value.token));
}
